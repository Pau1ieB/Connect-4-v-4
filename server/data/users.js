...
export const setupUserData=async ()=>{
    try{
        const file = await fs.readFile('./server/data/usersData','utf8');
        users = JSON.parse(file);
        userCount = users.reduce((count,users)=>(count>users.id)?count:users.id,-1)+1;
    }catch(err){
        await fs.writeFile(`./server/data/usersData`, JSON.stringify([]));
    }
}
...
export const saveUserData=async ()=>{
    await fs.writeFile(`./server/data/usersData`, JSON.stringify(users));
}